<template>
  <div
    class="collect-card-wrapper"
    :class="{ 'mr-xs': !isMobile, 'mb-xs': !isMobile, 'mb-lg': isMobile }"
  >
    <div class="image_wrap">
      ...
      <template v-else>
        <img
          ...
          :alt="`PetNi 陪你${animal.animal_title}`"
        />
      </template>
      <button class="bg_white delete_btn" @click="showDialogHandler">
    ...
    <div class="first_row">
      ...
      <q-icon size="1.5em">
        <template v-if="animal.animal_sex === 'F'">
          <img src="../../assets/icon/female.svg" />
        </template>
        <template v-else>
